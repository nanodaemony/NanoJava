...
使用了固定下边界通配符的列表拥有**一定程度的写操作**的能力，**List<? super E>** 是能够调用 **add 方法**的，因为在 addNumbers 方法中所 **add 的元素就是 Integer 类型的**，而传入的 list 不管是什么，都一定是 Integer 或其父类泛型的 List，这时 add 一个 Integer 元素是没有任何疑问的。
