...
ArrayDeque 要求对**数组大小**==必须是 **2^n**(2 的幂次方)==. 初始化容量如果不是 2 的幂次方也会被修改为 2 的幂次方. 
...
为何 "(tail + 1) & (elements.length - 1)" 能保证按照环形取得**正确的下一个索引值**呢? 这就与 ArrayDeque 对容量的特殊要求有关了. 下面对其正确性加以验证: 
