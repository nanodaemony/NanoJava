...
其中 **发送命令 + 响应结果** 是通过**网络**进行的，使用命令时应尽量**减少这个次数**。由于是**单线程**架构，所以如果命令过多或者前面的命令执行阻塞着，后面的命令需要在队列中**排队执行**。**慢查询**主要统计**执行命令**的时间。
...
<img src="assets/1563520443633.png" alt="1563520443633" style="zoom:46%;" />
